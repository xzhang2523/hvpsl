...
class PrefNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.problem_name == 'vlmop1':
            x = self.fc4(x)*2
        elif self.problem_name == 'vlmop2':
            x = self.fc4(x) * 2 * (1/args.n_var) * 1.2 - 1/args.n_var*1.2
        elif self.problem_name in ['zdt1', 'zdt2', 'dtlz2']:
            x = self.fc4(x)
        elif self.problem_name in ['lqr2', 'lqr3']:
            x = self.fc4(x)-1
        elif self.problem_name.startswith('RE'):
            x = self.fc4(x) * (self.ub - self.lb) + self.lb
        else:
            assert False
        
        # if self.problem_name == 'vlmop1':
        #     x = F.sigmoid(self.fc4(x))*2
        # elif self.problem_name == 'vlmop2':
        #     x = F.sigmoid(self.fc4(x)) * 2 * (1/args.n_var) * 1.2 - 1/args.n_var*1.2
        # elif self.problem_name in ['zdt1', 'zdt2', 'dtlz2']:
    ...
    for idx, (ji, prefi) in enumerate(zip(J, pref)):
        if decompose == 'tche':
            ...
            if m==2:
                ...
                loss_arr[idx] = rho**m * torch.sin(theta[idx, 2])
        elif decompose in ['hv2', 'hv2nograd']:
            if m ==2:
                loss_arr[idx] = torch.max((ji - args.ideal_point)**2 / prefi**2)
            elif m==3:
                loss_arr[idx] = torch.max((ji - args.ideal_point)**m / prefi**m) * torch.sin(theta[idx, 2])
        elif decompose == 'ls':
...
if __name__ == '__main__':
    ...
    # lr = args.lr
    n_iter = args.n_iter
    if problem_name in ['zdt1', 'zdt2', 'vlmop1', 'vlmop2'] or problem_name.startswith('RE2'):
        ideal_point = Tensor([0.0, 0.0])
    elif problem_name == 'lqr2':
        ideal_point = Tensor([1.50, 1.50])
    ...
    if problem_name in ['zdt1', 'zdt2', 'vlmop1', 'vlmop2'] or problem_name.startswith('RE2'):
        ...
        nadir_point = Tensor([1.2, 1.2, 1.2])
    elif problem_name.startswith('RE3'):
        nadir_point = Tensor([1.05, 1.05, 1.05])
    else:
        assert False, '{} for ref not set'.format(problem_name)
    ...
    with torch.no_grad():
        if args.n_obj == 3:
            ...
            x = model(pref)
            pref_np = pref.numpy()
            if problem_name in ['zdt1', 'zdt2'] or problem_name.startswith('RE'):
                pref_np = pref_np * 0.4
