...
class RE21():
    def __init__(self):
        ...
        self.ubound = Tensor(self.ubound)
        self.lbound = Tensor(self.lbound)
        pf_true = load_re_pf(self.problem_name)
        self.pf_min = Tensor(np.min(pf_true, axis=0))
        self.pf_max = Tensor(np.max(pf_true, axis=0)) 
        
        
    def evaluate(self, x):
        f = torch.zeros((len(x), self.n_objectives))
        ...
        f[:,0] = L * ((2 * x1) + np.sqrt(2.0) * x2 + torch.sqrt(x3) + x4)
        f[:,1] = ((F * L) / E) * ((2.0 / x1) + (2.0 * np.sqrt(2.0) / x2) - (2.0 * np.sqrt(2.0) / x3) + (2.0 / x4))
        for i in range(len(f)):
            f[i,:] = (f[i,:] - self.pf_min) / (self.pf_max - self.pf_min)
...
class RE24():
    ...
    def evaluate(self, x):
        ...
        x2 = x[:,1]
        f = torch.zeros((len(x), self.n_objectives))
        g = torch.zeros( (len(x), self.n_original_constraints))
...
class RE31():
    ...
    def evaluate(self, x):
        ...
        f = torch.zeros((len(x), self.n_objectives))
        g = torch.zeros((len(x), self.n_objectives))
        # g = np.zeros(self.n_original_constraints)
        ...
        # First original objective function
        f[:,0] = x1 * torch.sqrt(16.0 + (x3 * x3)) + x2 * torch.sqrt(1.0 + x3 * x3)
        # Second original objective function
        f[:,1] = (20.0 * torch.sqrt(16.0 + (x3 * x3))) / (x1 * x3)

        # Constraint functions 
        g[:,0] = 0.1 - f[:,0]
        g[:,1] = 100000.0 - f[:,1]
        g[:,2] = 100000 - ((80.0 * torch.sqrt(1.0 + x3 * x3)) / (x3 * x2))
        for i in range(len(x)):
            for j in range(self.n_original_constraints):
                if g[i,j] < 0:
                    g[i,j] = -g[i,j]
                else:
                    g[i,j] = 0
...
class RE33():
    ...
    def evaluate(self, x):
        ...
        g[:,2] = 1.0 - (2.22 * 1e-3 * x3 * (x2 * x2 * x2 - x1 * x1 * x1)) / ((x2 * x2 - x1 * x1) ** 2)
...
class RE34():
    ...
    def evaluate(self, x):
        ...
        f = (f - self.pf_min) / (self.pf_max - self.pf_min)
        return f
